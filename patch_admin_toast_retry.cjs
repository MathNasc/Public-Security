...
// 1. Add X, CheckCircle2, ArrowUpRight to lucide-react if not there
if (!code.includes('CheckCircle2')) {
  code = code.replace(
    /import \{ ShieldAlert/,
    'import { X, CheckCircle2, ArrowUpRight, ShieldAlert'
  );
}
...
code = code.replace(/alert\(\`Falha na automação: \$\{err\.message \|\| err\}\`\)/g, "showToast(`Falha na automação: ${err.message || err}`, 'error')");
code = code.replace(/alert\(\`Falha ao baixar amostra: \$\{err\.message \|\| err\}\`\)/g, "showToast(`Falha na automação: ${err.message || err}`, 'error')");

// 4. Inject the Toast UI right after <div className="max-w-7xl mx-auto space-y-8">
