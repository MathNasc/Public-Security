...
export class SespRrAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais SESP-RR",
      agency: "Secretaria de Segurança Pública (RR)",
      frequency: "Mensal",
      coverage: "RR",
      limitations: []
    };
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESP-RR",
      dataset: "Indicadores_Criminais_RR",
      url: `https://www.seguranca.rr.gov.br/dados/${currentYear}.csv`,
  ...
  parseRow(row: any): ParsedRecord[] | ParsedRecord | null {
    ...
    records.push({
      target: 'indicators',
      data: { id: crypto.randomUUID(), sourceId: 'SESP-RR', stateCode: 'RR', municipalityName: municipio, category: cat, sourceCategory: natureza, period: `${ano}-${mes}`, value: total, unit: 'ocorrencias' }
    });
    return records;
  }
}
