...
export interface SespRrCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const RR_CRIME_DEFINITIONS: SespRrCrimeDefinition[] = [
...
export class SespRrAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais da Secretaria de Estado da Segurança Pública de Roraima (SESP-RR)",
      agency: "SESP-RR",
      frequency: "Mensal",
      coverage: "RR",
      limitations: [
        "Os dados são compilados e disponibilizados mensalmente pela SESP-RR.",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESP-RR",
      dataset: "indicadores_municipais_rr",
      url: `https://www.seguranca.rr.gov.br/dados/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`sesp-rr-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: "Schema incompatível para SESP-RR. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
  ...
  mapCrimeToCanonical(crimeText: string): string {
    ...
    for (const def of RR_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...RR_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    if (code.length === 6 && code.startsWith('14')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const rrMunis: Record<string, string> = {
        'boa vista': '1400100',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Roraima' };
  }
