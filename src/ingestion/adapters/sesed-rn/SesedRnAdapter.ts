...
export class SesedRnAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais SESED-RN",
      agency: "Secretaria de Segurança Pública (RN)",
      frequency: "Mensal",
      coverage: "RN",
      limitations: []
    };
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESED-RN",
      dataset: "Indicadores_Criminais_RN",
      url: `https://www.seguranca.rn.gov.br/dados/${currentYear}.csv`,
  ...
  parseRow(row: any): ParsedRecord[] | ParsedRecord | null {
    ...
    records.push({
      target: 'indicators',
      data: { id: crypto.randomUUID(), sourceId: 'SESED-RN', stateCode: 'RN', municipalityName: municipio, category: cat, sourceCategory: natureza, period: `${ano}-${mes}`, value: total, unit: 'ocorrencias' }
    });
    return records;
  }
}
