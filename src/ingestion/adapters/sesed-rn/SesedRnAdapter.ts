...
export interface SesedRnCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const RN_CRIME_DEFINITIONS: SesedRnCrimeDefinition[] = [
  // Condutas Violentas Letais Intencionais (CVLI) / Mortes Violentas
  ...
  { key: 'cvli', category: 'homicide', label: 'Condutas Violentas Letais Intencionais' },
  { key: 'condutas violentas letais intencionais', category: 'homicide', label: 'Condutas Violentas Letais Intencionais' },
...
export class SesedRnAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas da Secretaria de Estado da Segurança Pública e da Defesa Social do Rio Grande do Norte (SESED-RN)",
      agency: "SESED-RN",
      frequency: "Mensal",
      coverage: "RN",
      limitations: [
        "Os dados são compilados mensalmente pela Coordenadoria de Informações Estatísticas e Análise Criminal (COINE/SESED-RN).",
        "Indicadores de CVLI (Condutas Violentas Letais Intencionais) congregam Homicídio Doloso, Feminicídio, Latrocínio e Lesão Corporal Seguida de Morte."
      ]
    };
  }
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESED-RN",
      dataset: "indicadores_municipais_rn",
      url: `https://www.seguranca.rn.gov.br/estatisticas/dados-consolidados-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`sesed-rn-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    // Formato 1: Wide / Matricial Municipal (Colunas de crimes por linha de município/AISP/RISP)
    const hasMuni = normHeaders.some(h => 
      h.includes('municipio') || h.includes('cidade') || h.includes('cod_ibge') || h.includes('ibge') || h.includes('aisp') || h.includes('risp') || h.includes('delegacia')
    );
    ...
    return {
      valid: false,
      error: "Schema incompatível para SESED-RN. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
      missingColumns: ['municipio / cod_ibge', 'ano / mes', 'natureza / crimes']
    };
  }

  /**
   * Mapeia qualquer natureza criminal do Rio Grande do Norte para a taxonomia canônica
   */
  mapCrimeToCanonical(crimeText: string): string {
    ...
    // 1. Verificação exata
    for (const def of RN_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...RN_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    const rawName = row['municipio'] || row['Municipio'] || row['MUNICIPIO'] || row['cidade'] || row['Cidade'] || row['aisp'] || row['risp'] || '';

    let code = String(rawIbge).trim();
    let name = String(rawName).trim();

    if (code.length === 6 && code.startsWith('24')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const rnMunis: Record<string, string> = {
        'natal': '2408102',
        'mossoro': '2408003',
        'parnamirim': '2403251',
        'sao goncalo do amarante': '2412005',
        'ceara-mirim': '2402600',
        'ceara mirim': '2402600',
        'macaiba': '2407104',
        'caico': '2402006',
        'acu': '2400208',
        'sao jose de mipibu': '2411502',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Rio Grande do Norte' };
  }
  ...
  parseRow(row: any): ParsedRecord[] | ParsedRecord | null {
    ...
    const cvliTotal = this.parseNumeric(normalizedKeys['cvli'] || normalizedKeys['total_cvli'] || normalizedKeys['mortes_violentas'] || normalizedKeys['condutas_violentas_letais_intencionais']);
    ...
    if (finalHomicide > 0 || (normalizedKeys['homicidio_doloso'] !== undefined)) {
      records.push({
        ...
        data: {
          ...
          municipalityCode: muniCode,
          category: 'homicide',
          sourceCategory: 'Condutas Violentas Letais Intencionais (CVLI)',
          period,
          value: finalHomicide,
          unit: 'vitimas'
        }
