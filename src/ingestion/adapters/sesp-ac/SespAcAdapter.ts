...
export interface SespAcCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const AC_CRIME_DEFINITIONS: SespAcCrimeDefinition[] = [
...
export class SespAcAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais da Secretaria de Estado de Justiça e Segurança Pública do Acre (SESP-AC)",
      agency: "SESP-AC",
      frequency: "Mensal",
      coverage: "AC",
      limitations: [
        "Os dados são compilados e disponibilizados mensalmente pela SESP-AC.",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESP-AC",
      dataset: "indicadores_municipais_ac",
      url: `https://www.seguranca.ac.gov.br/dados/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`sesp-ac-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: "Schema incompatível para SESP-AC. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
  ...
  mapCrimeToCanonical(crimeText: string): string {
    ...
    for (const def of AC_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...AC_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    if (!code || code === '0') {
      ...
      const acMunis: Record<string, string> = {
        'rio branco': '1200401',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Acre' };
  }
