...
export interface SesdecRoCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const RO_CRIME_DEFINITIONS: SesdecRoCrimeDefinition[] = [
...
export class SesdecRoAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais da Secretaria de Estado da Segurança, Defesa e Cidadania de Rondônia (SESDEC-RO)",
      agency: "SESDEC-RO",
      frequency: "Mensal",
      coverage: "RO",
      limitations: [
        "Os dados são compilados e disponibilizados mensalmente pela SESDEC-RO.",
        "Indicadores de CVLI (Crimes Violentos Letais Intencionais) agregam Homicídio Doloso, Feminicídio, Latrocínio e Lesão Corporal Seguida de Morte."
      ]
    };
  }
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESDEC-RO",
      dataset: "indicadores_municipais_ro",
      url: `https://www.seguranca.ro.gov.br/dados/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`sesdec-ro-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    const hasMuni = normHeaders.some(h => 
      h.includes('municipio') || h.includes('cidade') || h.includes('cod_ibge') || h.includes('ibge') || h.includes('delegacia')
    );
    ...
    return {
      valid: false,
      error: "Schema incompatível para SESDEC-RO. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
  ...
  mapCrimeToCanonical(crimeText: string): string {
    ...
    for (const def of RO_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...RO_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    if (code.length === 6 && code.startsWith('11')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const roMunis: Record<string, string> = {
        'porto velho': '1100205',
        'ji parana': '1100122',
        'ariquemes': '1100023',
        'vilhena': '1100304',
        ...
        'espigao do oeste': '1100098',
        'alta floresta d oeste': '1100015',
        'alta floresta do oeste': '1100015',
        'candeias do jamari': '1100801',
        'sao miguel do guapore': '1100320',
        'nova mamore': '1100338',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Rondônia' };
  }
