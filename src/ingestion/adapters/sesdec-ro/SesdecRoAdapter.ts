...
export class SesdecRoAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais SESDEC-RO",
      agency: "Secretaria de Segurança Pública (RO)",
      frequency: "Mensal",
      coverage: "RO",
      limitations: []
    };
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESDEC-RO",
      dataset: "Indicadores_Criminais_RO",
      url: `https://www.seguranca.ro.gov.br/dados/${currentYear}.csv`,
  ...
  parseRow(row: any): ParsedRecord[] | ParsedRecord | null {
    ...
    records.push({
      target: 'indicators',
      data: { id: crypto.randomUUID(), sourceId: 'SESDEC-RO', stateCode: 'RO', municipalityName: municipio, category: cat, sourceCategory: natureza, period: `${ano}-${mes}`, value: total, unit: 'ocorrencias' }
    });
    return records;
  }
}
