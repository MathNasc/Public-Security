...
export interface SejuspMsCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const MS_CRIME_DEFINITIONS: SejuspMsCrimeDefinition[] = [
...
export class SejuspMsAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais da Secretaria de Estado de Justiça e Segurança Pública de Mato Grosso do Sul (SEJUSP-MS)",
      agency: "SEJUSP-MS",
      frequency: "Mensal",
      coverage: "MS",
      limitations: [
        "Os dados são compilados e disponibilizados mensalmente pela SEJUSP-MS.",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SEJUSP-MS",
      dataset: "indicadores_municipais_ms",
      url: `https://www.sejusp.ms.gov.br/dados/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`sejusp-ms-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: "Schema incompatível para SEJUSP-MS. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
  ...
  mapCrimeToCanonical(crimeText: string): string {
    ...
    for (const def of MS_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...MS_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    if (code.length === 6 && code.startsWith('50')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const msMunis: Record<string, string> = {
    ...
    return { code: code || 'UNKNOWN', name: name || 'Mato Grosso do Sul' };
  }
