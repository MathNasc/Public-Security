...
export interface SspMaCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const MA_CRIME_DEFINITIONS: SspMaCrimeDefinition[] = [
  ...
  { key: 'morte por intervencao policial', category: 'homicide', label: 'Morte por Intervenção Policial' },
...
export class SspMaAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas da Segurança Pública do Maranhão (SSP-MA)",
      agency: "SSP-MA",
      frequency: "Mensal",
      coverage: "MA",
      limitations: [
        "Os dados são consolidados mensalmente por município ou delegacia pelo Centro Integrado de Estatística e Análise Criminal da SSP-MA.",
        "Indicadores de CVLI incluem Homicídio Doloso, Latrocínio, Feminicídio e Lesão Corporal Seguida de Morte."
      ]
    };
  }
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SSP-MA",
      dataset: "indicadores_municipais_ma",
      url: `https://www.seguranca.ma.gov.br/estatisticas/dados-consolidados-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-ma-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    const hasMuni = normHeaders.some(h => 
      h.includes('municipio') || h.includes('cidade') || h.includes('cod_ibge') || h.includes('ibge') || h.includes('cisp') || h.includes('delegacia')
    );
    ...
    return {
      valid: false,
      error: "Schema incompatível para SSP-MA. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
      missingColumns: ['municipio / cod_ibge', 'ano / mes', 'natureza / crimes']
    };
  }

  /**
   * Mapeia qualquer natureza criminal ou delito do Maranhão para a taxonomia canônica
   */
  mapCrimeToCanonical(crimeText: string): string {
    ...
    // 1. Verificação exata
    for (const def of MA_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...MA_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  /**
   * Helper para extrair código do município ou normalizar nome do MA
   */
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    const rawName = row['municipio'] || row['Municipio'] || row['MUNICIPIO'] || row['cidade'] || row['Cidade'] || row['delegacia'] || '';

    let code = String(rawIbge).trim();
    let name = String(rawName).trim();

    if (code.length === 6 && code.startsWith('21')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const maMunis: Record<string, string> = {
        'sao luis': '2111300',
        'imperatriz': '2105302',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Maranhão' };
  }
