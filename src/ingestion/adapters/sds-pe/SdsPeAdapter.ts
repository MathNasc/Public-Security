...
const PE_CRIME_DEFINITIONS: PeCrimeMapping[] = [
  ...
  { key: 'mortes violentas intencionais', category: 'homicide', subcategory: 'cvli', unit: 'vitimas' },
  { key: 'mvi', category: 'homicide', subcategory: 'cvli', unit: 'vitimas' },
...
const PE_MONTH_NAMES_MAP: Record<string, number> = {
...
/**
 * Adapter oficial para SDS-PE (Secretaria de Defesa Social do Estado de Pernambuco).
 * Suporta os relatórios estatísticos municipais com métricas de CVLI e CVP consolidados pela GGACE.
 */
export class SdsPeAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Secretaria de Defesa Social de Pernambuco - SDS-PE",
      agency: "SDS-PE",
      frequency: "Mensal",
      coverage: "PE",
      limitations: [
        "Estatísticas de CVLI (Crimes Violentos Letais Intencionais) e CVP (Crimes Violentos contra o Patrimônio) consolidadas pela GGACE",
        "Cobertura total de 100% dos 185 municípios do Estado de Pernambuco + Distrito Estadual de Fernando de Noronha",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    const url = `https://www.sds.pe.gov.br/estatisticas`;

    return {
      source: "SDS-PE",
      dataset: "indicadores_municipais_pe",
      url,
      version,
      checksum: crypto.createHash('sha256').update(`sds-pe-${version}`).digest('hex')
    };
  }

  async download(destinationPath: string): Promise<string> {
    console.log(`[SDS-PE Adapter] Ingestão pronta para ${destinationPath}`);
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: `Schema incompatível para SDS-PE. Colunas ausentes: ${missing.join(', ')}`,
  ...
  mapCrimeToCanonical(naturezaStr: string): CanonicalCategory {
    ...
    const found = PE_CRIME_DEFINITIONS.find(m => m.key === clean || clean.includes(m.key));
    ...
    if (clean.includes('cvp') || clean.includes('roubo') || clean.includes('transeunte') || clean.includes('assalto') || clean.includes('coletivo')) return 'robbery';
  ...
  parseRow(row: any): ParsedRecord[] | null {
    ...
    if (PE_MONTH_NAMES_MAP[cleanMesStr]) {
      mesNum = PE_MONTH_NAMES_MAP[cleanMesStr];
    } else {
    ...
    if (rawNatureza) {
      ...
      const def = PE_CRIME_DEFINITIONS.find(d => d.key === cleanNaturezaKey || cleanNaturezaKey.includes(d.key));
    ...
    const vehRob = this.extractNumericValue(normalizedRow, 'roubo_veiculo') || this.extractNumericValue(normalizedRow, 'roubo_de_veiculo') || this.extractNumericValue(normalizedRow, 'cvp_veiculo');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
    ...
    const robTran = this.extractNumericValue(normalizedRow, 'cvp_transeunte') || this.extractNumericValue(normalizedRow, 'roubo_transeunte');
