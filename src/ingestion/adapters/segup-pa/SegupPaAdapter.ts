...
export class SegupPaAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais SEGUP-PA",
      agency: "Secretaria de Segurança Pública (PA)",
      frequency: "Mensal",
      coverage: "PA",
      limitations: []
    };
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SEGUP-PA",
      dataset: "Indicadores_Criminais_PA",
      url: `https://www.seguranca.pa.gov.br/dados/${currentYear}.csv`,
  ...
  parseRow(row: any): ParsedRecord[] | ParsedRecord | null {
    ...
    records.push({
      target: 'indicators',
      data: { id: crypto.randomUUID(), sourceId: 'SEGUP-PA', stateCode: 'PA', municipalityName: municipio, category: cat, sourceCategory: natureza, period: `${ano}-${mes}`, value: total, unit: 'ocorrencias' }
    });
    return records;
  }
}
