...
const ES_CRIME_DEFINITIONS: EsCrimeMapping[] = [
  // Crimes Violentos Letais Intencionais (CVLI / MVLI) - Mortes Violentas
  ...
  { key: 'mvli', category: 'homicide', subcategory: 'cvli', unit: 'vitimas' },
  { key: 'mortes violentas por letalidade intencional', category: 'homicide', subcategory: 'cvli', unit: 'vitimas' },
  ...
  { key: 'roubo transcol', category: 'robbery', subcategory: 'roubo_onibus', unit: 'ocorrencias' },
...
const ES_MONTH_NAMES_MAP: Record<string, number> = {
...
/**
 * Adapter oficial para SESP-ES (Secretaria de Estado da Segurança Pública e Defesa Social do Espírito Santo).
 * Suporta relatórios estatísticos consolidados pelo Observatório da Segurança Pública do ES (OSP-ES) e IJSN.
 */
export class SespEsAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Secretaria de Estado da Segurança Pública e Defesa Social do Espírito Santo - SESP-ES",
      agency: "SESP-ES",
      frequency: "Mensal",
      coverage: "ES",
      limitations: [
        "Estatísticas consolidadas pelo Observatório da Segurança Pública do ES (OSP-ES) e IJSN",
        "Cobertura total de 100% dos 78 municípios do Estado do Espírito Santo",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    const url = `https://dados.es.gov.br/`;

    return {
      source: "SESP-ES",
      dataset: "indicadores_municipais_es",
      url,
      version,
      checksum: crypto.createHash('sha256').update(`sesp-es-${version}`).digest('hex')
    };
  }

  async download(destinationPath: string): Promise<string> {
    console.log(`[SESP-ES Adapter] Ingestão pronta para ${destinationPath}`);
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    const hasLocation = cleanHeaders.some(h => 
      ['cod_ibge', 'codigo_ibge', 'ibge', 'municipio', 'nome_municipio', 'cidade', 'risp', 'aisp'].includes(h)
    );
    ...
    // Formato 1: Wide (colunas de crimes como homicidio, mvli, roubo_veiculo, furto_veiculo, etc.)
    const hasWideCols = cleanHeaders.some(h => 
      ['homicidio_doloso', 'homicidio', 'mvli', 'latrocinio', 'roubo_veiculo', 'roubo de veiculo', 'furto_veiculo', 'furto de veiculo', 'roubo_pessoa', 'roubo_transeunte', 'furto', 'estupro', 'trafico_drogas', 'cvli', 'cvp'].includes(h)
    );
    ...
    if (!hasLocation) missing.push('cod_ibge / municipio / risp');
    ...
    return {
      valid: false,
      error: `Schema incompatível para SESP-ES. Colunas ausentes: ${missing.join(', ')}`,
  ...
  mapCrimeToCanonical(naturezaStr: string): CanonicalCategory {
    ...
    const exact = ES_CRIME_DEFINITIONS.find(m => m.key === clean);
    ...
    const found = ES_CRIME_DEFINITIONS.find(m => clean.includes(m.key));
    if (found) return found.category;

    if (clean.includes('homicidio') || clean.includes('morte') || clean.includes('latrocinio') || clean.includes('feminicidio') || clean.includes('mvli') || clean.includes('cvli')) return 'homicide';
    ...
    if (clean.includes('estupro') || clean.includes('sexual') || clean.includes('costumes')) return 'sexual_crime';
    if (clean.includes('lesao') || clean.includes('agressao') || clean.includes('violencia')) return 'bodily_harm';
    if (clean.includes('droga') || clean.includes('entorpecente') || clean.includes('toxico')) return 'drug_related';
    if (clean.includes('roubo') || clean.includes('transeunte') || clean.includes('pessoa') || clean.includes('assalto') || clean.includes('coletivo') || clean.includes('transcol') || clean.includes('cvp')) return 'robbery';
  ...
  parseRow(row: any): ParsedRecord[] | null {
    ...
    if (ES_MONTH_NAMES_MAP[cleanMesStr]) {
      mesNum = ES_MONTH_NAMES_MAP[cleanMesStr];
    } else {
    ...
    if (rawNatureza) {
      ...
      const def = ES_CRIME_DEFINITIONS.find(d => d.key === cleanNaturezaKey || cleanNaturezaKey.includes(d.key));
    ...
    // 1. Homicide (Homicídio Doloso, Latrocínio, Feminicídio, Lesão Morte ou MVLI/CVLI)
    ...
    const mvliVal = this.extractNumericValue(normalizedRow, 'mvli') || this.extractNumericValue(normalizedRow, 'cvli');

    const totalHomicide = mvliVal > 0 ? mvliVal : (homVal + latroVal + femVal + lesaoMorteVal);
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
        ...
        sourceCategory: mvliVal > 0 ? 'mvli' : 'homicidio_doloso',
        period: periodStr,
        value: totalHomicide,
        unit: 'vitimas',
        granularity: 'municipality',
    ...
    // 4. Robbery (Roubo a Pessoa / Transeunte / Coletivo / Transcol / Comércio / Residência / CVP Geral)
    const robPessoa = this.extractNumericValue(normalizedRow, 'roubo_pessoa') || this.extractNumericValue(normalizedRow, 'roubo_a_pessoa') || this.extractNumericValue(normalizedRow, 'roubo_transeunte') || this.extractNumericValue(normalizedRow, 'roubo_a_transeunte');
    const robCom = this.extractNumericValue(normalizedRow, 'roubo_comercio') || this.extractNumericValue(normalizedRow, 'roubo_a_comercio') || this.extractNumericValue(normalizedRow, 'roubo_estabelecimento_comercial');
    ...
    const robBus = this.extractNumericValue(normalizedRow, 'roubo_coletivo') || this.extractNumericValue(normalizedRow, 'roubo_onibus') || this.extractNumericValue(normalizedRow, 'roubo_transcol');
    ...
    const aggregatedSub = (robPessoa + robCom + robRes + robBus);
