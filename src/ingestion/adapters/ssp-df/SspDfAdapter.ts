...
const DF_CRIME_DEFINITIONS: DfCrimeMapping[] = [
  ...
  { key: 'roubo com morte', category: 'homicide', subcategory: 'latrocinio', unit: 'vitimas' },
  ...
  { key: 'lei maria da penha', category: 'bodily_harm', subcategory: 'violencia_domestica', unit: 'ocorrencias' },
  ...
  // Crimes contra o Patrimônio - Roubos (CVP)
  ...
  { key: 'furto em interior de veiculo', category: 'theft', subcategory: 'furto_interior_veiculo', unit: 'ocorrencias' },
  ...
  { key: 'uso e porte de drogas', category: 'drug_related', subcategory: 'porte_drogas', unit: 'ocorrencias' },
  { key: 'porte de drogas', category: 'drug_related', subcategory: 'porte_drogas', unit: 'ocorrencias' },
  ...
  { key: 'porte ilegal de arma', category: 'other', subcategory: 'porte_arma', unit: 'ocorrencias' },
  { key: 'mandados de prisao cumpridos', category: 'other', subcategory: 'mandados_cumpridos', unit: 'pessoas' },
...
const DF_MONTH_NAMES_MAP: Record<string, number> = {
...
/**
 * Adapter oficial para SSP-DF (Secretaria de Estado de Segurança Pública do Distrito Federal).
 * Suporta a estrutura de Regiões Administrativas (RAs) e indicadores consolidados pela SGI / GEAC.
 */
export class SspDfAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Secretaria de Estado de Segurança Pública do Distrito Federal - SSP-DF",
      agency: "SSP-DF",
      frequency: "Mensal",
      coverage: "DF",
      limitations: [
        "Estatísticas consolidadas pela Subsecretaria de Gestão da Informação (SGI / GEAC)",
        "Cobertura total das 35 Regiões Administrativas (RAs) do Distrito Federal",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    const url = `https://dados.df.gov.br/`;

    return {
      source: "SSP-DF",
      dataset: "indicadores_criminais_df",
      url,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-df-${version}`).digest('hex')
    };
  }

  async download(destinationPath: string): Promise<string> {
    console.log(`[SSP-DF Adapter] Ingestão pronta para ${destinationPath}`);
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    const hasLocation = cleanHeaders.some(h => 
      ['regiao_administrativa', 'ra', 'cidade', 'municipio', 'cod_ibge', 'codigo_ibge', 'ibge', 'local'].includes(h)
    );
    ...
    // Formato 1: Wide (colunas de crimes como homicidio_doloso, roubo_veiculo, furto_veiculo, etc.)
    const hasWideCols = cleanHeaders.some(h => 
      ['homicidio_doloso', 'homicidio', 'latrocinio', 'roubo_veiculo', 'roubo de veiculo', 'furto_veiculo', 'furto de veiculo', 'roubo_transeunte', 'furto', 'estupro', 'trafico_drogas', 'cvli', 'cvp'].includes(h)
    );
    ...
    if (!hasLocation) missing.push('regiao_administrativa / ra / municipio');
    ...
    return {
      valid: false,
      error: `Schema incompatível para SSP-DF. Colunas ausentes: ${missing.join(', ')}`,
  ...
  mapCrimeToCanonical(naturezaStr: string): CanonicalCategory {
    ...
    const exact = DF_CRIME_DEFINITIONS.find(m => m.key === clean);
    ...
    const found = DF_CRIME_DEFINITIONS.find(m => clean.includes(m.key));
    if (found) return found.category;

    if (clean.includes('homicidio') || clean.includes('morte') || clean.includes('latrocinio') || clean.includes('feminicidio') || clean.includes('cvli')) return 'homicide';
    ...
    if (clean.includes('roubo') || clean.includes('transeunte') || clean.includes('assalto') || clean.includes('coletivo') || clean.includes('pessoa') || clean.includes('cvp')) return 'robbery';
  ...
  parseRow(row: any): ParsedRecord[] | null {
    ...
    if (DF_MONTH_NAMES_MAP[cleanMesStr]) {
      mesNum = DF_MONTH_NAMES_MAP[cleanMesStr];
    } else {
    ...
    // Identificação geográfica (Distrito Federal / Regiões Administrativas)
    const rawIbge = normalizedRow['cod_ibge'] || normalizedRow['codigo_ibge'] || normalizedRow['ibge'];
    const municipalityName = normalizedRow['regiao_administrativa'] || normalizedRow['ra'] || normalizedRow['municipio'] || normalizedRow['cidade'] || 'Brasília';

    let municipalityCode: string = '5300108'; // Default IBGE Brasília/DF
    if (municipalityName && municipalityName.toLowerCase() !== 'brasilia' && municipalityName.toLowerCase() !== 'distrito federal' && municipalityName.toLowerCase() !== 'df') {
      const raSlug = municipalityName.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').replace(/^_|_$/g, '');
      municipalityCode = `5300108-${raSlug}`;
    } else if (rawIbge) {
    ...
    if (rawNatureza) {
      ...
      const def = DF_CRIME_DEFINITIONS.find(d => d.key === cleanNaturezaKey || cleanNaturezaKey.includes(d.key));
    ...
    // 1. Homicide (Homicídio Doloso, Latrocínio, Feminicídio, Lesão Morte ou CVLI)
    const homVal = this.extractNumericValue(normalizedRow, 'homicidio_doloso') || this.extractNumericValue(normalizedRow, 'homicidio');
    const latroVal = this.extractNumericValue(normalizedRow, 'latrocinio') || this.extractNumericValue(normalizedRow, 'roubo_seguido_de_morte') || this.extractNumericValue(normalizedRow, 'roubo_com_morte');
    ...
    const cvliVal = this.extractNumericValue(normalizedRow, 'cvli');
    ...
    // 2. Vehicle Robbery (Roubo de Veículo)
    const vehRob = this.extractNumericValue(normalizedRow, 'roubo_veiculo') || this.extractNumericValue(normalizedRow, 'roubo_de_veiculo') || this.extractNumericValue(normalizedRow, 'roubo_auto') || this.extractNumericValue(normalizedRow, 'roubo_de_veiculos');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
    ...
    // 4. Robbery (Roubo a Transeunte / Coletivo / Comércio / Residência / CVP Geral)
    const robTran = this.extractNumericValue(normalizedRow, 'roubo_transeunte') || this.extractNumericValue(normalizedRow, 'roubo_a_transeunte') || this.extractNumericValue(normalizedRow, 'roubo_pessoa');
    const robCom = this.extractNumericValue(normalizedRow, 'roubo_comercio') || this.extractNumericValue(normalizedRow, 'roubo_a_comercio');
    const robRes = this.extractNumericValue(normalizedRow, 'roubo_residencia') || this.extractNumericValue(normalizedRow, 'roubo_a_residencia');
    const robBus = this.extractNumericValue(normalizedRow, 'roubo_coletivo') || this.extractNumericValue(normalizedRow, 'roubo_em_transporte_coletivo');
    const robGer = this.extractNumericValue(normalizedRow, 'roubo') || this.extractNumericValue(normalizedRow, 'roubo_geral') || this.extractNumericValue(normalizedRow, 'cvp');

    const aggregatedSub = (robTran + robCom + robRes + robBus);
    const totalRobbery = aggregatedSub > 0 ? aggregatedSub : robGer;
    ...
    // 5. Theft (Furto Geral)
    const theftVal = this.extractNumericValue(normalizedRow, 'furto') || this.extractNumericValue(normalizedRow, 'furto_geral') || this.extractNumericValue(normalizedRow, 'furtos_diversos') || this.extractNumericValue(normalizedRow, 'furto_transeunte');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
