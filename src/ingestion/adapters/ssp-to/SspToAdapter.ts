...
export interface SspToCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const TO_CRIME_DEFINITIONS: SspToCrimeDefinition[] = [
...
export class SspToAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais da Secretaria de Estado da Segurança Pública do Tocantins (SSP-TO)",
      agency: "SSP-TO",
      frequency: "Mensal",
      coverage: "TO",
      limitations: [
        "Os dados são compilados e disponibilizados mensalmente pela SSP-TO.",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SSP-TO",
      dataset: "indicadores_municipais_to",
      url: `https://www.seguranca.to.gov.br/dados/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-to-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: "Schema incompatível para SSP-TO. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
  ...
  mapCrimeToCanonical(crimeText: string): string {
    ...
    for (const def of TO_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...TO_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    if (code.length === 6 && code.startsWith('17')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const toMunis: Record<string, string> = {
        'palmas': '1721000',
        'araguaina': '1702109',
        'gurupi': '1709500',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Tocantins' };
  }
