...
export interface SspAlCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const AL_CRIME_DEFINITIONS: SspAlCrimeDefinition[] = [
...
export class SspAlAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas da Secretaria de Estado da Segurança Pública de Alagoas (SSP-AL)",
      agency: "SSP-AL",
      frequency: "Mensal",
      coverage: "AL",
      limitations: [
        "Os dados são compilados pela Chefia de Informações Estatísticas e Georreferenciadas (CIEG/SSP-AL).",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SSP-AL",
      dataset: "indicadores_municipais_al",
      url: `https://dados.al.gov.br/dataset/seguranca-publica/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-al-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    // Formato 1: Wide / Matricial Municipal (Colunas de crimes por linha de município/AISP/CISP)
    const hasMuni = normHeaders.some(h => 
      h.includes('municipio') || h.includes('cidade') || h.includes('cod_ibge') || h.includes('ibge') || h.includes('cisp') || h.includes('aisp') || h.includes('delegacia')
    );
    ...
    return {
      valid: false,
      error: "Schema incompatível para SSP-AL. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
      missingColumns: ['municipio / cod_ibge', 'ano / mes', 'natureza / crimes']
    };
  }

  /**
   * Mapeia qualquer natureza criminal de Alagoas para a taxonomia canônica
   */
  mapCrimeToCanonical(crimeText: string): string {
    ...
    // 1. Verificação exata
    for (const def of AL_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...AL_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    const rawName = row['municipio'] || row['Municipio'] || row['MUNICIPIO'] || row['cidade'] || row['Cidade'] || row['cisp'] || row['aisp'] || '';

    let code = String(rawIbge).trim();
    let name = String(rawName).trim();

    if (code.length === 6 && code.startsWith('27')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const alMunis: Record<string, string> = {
        'maceio': '2704302',
        'arapiraca': '2700300',
        'rio largo': '2707701',
        'palmeira dos indios': '2706703',
        'teotonio vilela': '2709152',
        'sao miguel dos campos': '2708907',
        'coruripe': '2702306',
        'delmiro gouveia': '2702405',
        'marechal deodoro': '2704708',
        'santana do ipanema': '2708600',
        'uniao dos palmares': '2709301',
        'penedo': '2706901',
        'pilar': '2708006',
        'sao luis do quitunde': '2708808',
        'sao jose da tapera': '2708709',
        'campo alegre': '2702207',
        'murici': '2705200',
        'atalaia': '2700508',
        'girau do ponciano': '2703007'
      };
    ...
    return { code: code || 'UNKNOWN', name: name || 'Alagoas' };
  }
