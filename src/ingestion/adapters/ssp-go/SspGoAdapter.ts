...
const GO_CRIME_DEFINITIONS: GoCrimeMapping[] = [
  ...
  { key: 'roubo em propriedade rural', category: 'robbery', subcategory: 'roubo_rural', unit: 'ocorrencias' },
  { key: 'roubo rural', category: 'robbery', subcategory: 'roubo_rural', unit: 'ocorrencias' },
  ...
  { key: 'furto em propriedade rural', category: 'theft', subcategory: 'furto_rural', unit: 'ocorrencias' },
  ...
  { key: 'foragidos recapturados', category: 'other', subcategory: 'recapturas', unit: 'pessoas' },
...
const GO_MONTH_NAMES_MAP: Record<string, number> = {
...
/**
 * Adapter oficial para SSP-GO (Secretaria de Estado da Segurança Pública de Goiás).
 * Suporta os relatórios estatísticos consolidados pelo OSPEGO / GOSP para os 246 municípios goianos.
 */
export class SspGoAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Secretaria de Estado da Segurança Pública de Goiás - SSP-GO",
      agency: "SSP-GO",
      frequency: "Mensal",
      coverage: "GO",
      limitations: [
        "Estatísticas consolidadas pelo Observatório de Segurança Pública do Estado de Goiás (OSPEGO)",
        "Cobertura total de 100% dos 246 municípios do Estado de Goiás",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    const url = `https://dadosabertos.ssp.go.gov.br/`;

    return {
      source: "SSP-GO",
      dataset: "indicadores_municipais_go",
      url,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-go-${version}`).digest('hex')
    };
  }

  async download(destinationPath: string): Promise<string> {
    console.log(`[SSP-GO Adapter] Ingestão pronta para ${destinationPath}`);
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    const hasLocation = cleanHeaders.some(h => 
      ['cod_ibge', 'codigo_ibge', 'ibge', 'municipio', 'nome_municipio', 'cidade', 'risp', 'crpm'].includes(h)
    );
    ...
    // Formato 1: Wide (colunas de crimes como homicidio, roubo_veiculo, furto_veiculo, etc.)
    ...
    return {
      valid: false,
      error: `Schema incompatível para SSP-GO. Colunas ausentes: ${missing.join(', ')}`,
  ...
  mapCrimeToCanonical(naturezaStr: string): CanonicalCategory {
    ...
    const exact = GO_CRIME_DEFINITIONS.find(m => m.key === clean);
    ...
    const found = GO_CRIME_DEFINITIONS.find(m => clean.includes(m.key));
  ...
  parseRow(row: any): ParsedRecord[] | null {
    ...
    if (GO_MONTH_NAMES_MAP[cleanMesStr]) {
      mesNum = GO_MONTH_NAMES_MAP[cleanMesStr];
    } else {
    ...
    if (rawNatureza) {
      ...
      const def = GO_CRIME_DEFINITIONS.find(d => d.key === cleanNaturezaKey || cleanNaturezaKey.includes(d.key));
    ...
    // 4. Robbery (Roubo a Transeunte / Coletivo / Comércio / Residência / Rural / Geral)
    ...
    const robBus = this.extractNumericValue(normalizedRow, 'roubo_coletivo') || this.extractNumericValue(normalizedRow, 'roubo_onibus');
    const robRural = this.extractNumericValue(normalizedRow, 'roubo_rural') || this.extractNumericValue(normalizedRow, 'roubo_propriedade_rural');
    ...
    const aggregatedSub = (robTran + robCom + robRes + robBus + robRural);
