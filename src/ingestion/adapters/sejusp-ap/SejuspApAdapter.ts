...
export interface SejuspApCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const AP_CRIME_DEFINITIONS: SejuspApCrimeDefinition[] = [
...
export class SejuspApAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais da Secretaria de Estado da Justiça e Segurança Pública do Amapá (SEJUSP-AP)",
      agency: "SEJUSP-AP",
      frequency: "Mensal",
      coverage: "AP",
      limitations: [
        "Os dados são compilados e disponibilizados mensalmente pela SEJUSP-AP.",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SEJUSP-AP",
      dataset: "indicadores_municipais_ap",
      url: `https://www.seguranca.ap.gov.br/dados/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`sejusp-ap-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: "Schema incompatível para SEJUSP-AP. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
  ...
  mapCrimeToCanonical(crimeText: string): string {
    ...
    for (const def of AP_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...AP_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    if (code.length === 6 && code.startsWith('16')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const apMunis: Record<string, string> = {
        'macapa': '1600303',
        'santana': '1600600',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Amapá' };
  }
