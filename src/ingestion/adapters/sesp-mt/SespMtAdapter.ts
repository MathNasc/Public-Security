...
export interface SespMtCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const MT_CRIME_DEFINITIONS: SespMtCrimeDefinition[] = [
...
export class SespMtAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas da Secretaria de Estado de Segurança Pública de Mato Grosso (SESP-MT)",
      agency: "SESP-MT",
      frequency: "Mensal",
      coverage: "MT",
      limitations: [
        "Os dados são compilados pelo Observatório de Segurança Pública da SESP-MT.",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SESP-MT",
      dataset: "indicadores_municipais_mt",
      url: `https://www.seguranca.mt.gov.br/dados/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`sesp-mt-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    const hasMuni = normHeaders.some(h => 
      h.includes('municipio') || h.includes('cidade') || h.includes('cod_ibge') || h.includes('ibge') || h.includes('risp') || h.includes('delegacia')
    );
    ...
    return {
      valid: false,
      error: "Schema incompatível para SESP-MT. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
  ...
  mapCrimeToCanonical(crimeText: string): string {
    ...
    for (const def of MT_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...MT_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    if (code.length === 6 && code.startsWith('51')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const mtMunis: Record<string, string> = {
        'cuiaba': '5103403',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Mato Grosso' };
  }
