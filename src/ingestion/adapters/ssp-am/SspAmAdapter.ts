...
export interface SspAmCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const AM_CRIME_DEFINITIONS: SspAmCrimeDefinition[] = [
  ...
  // Crimes Contra a Pessoa / Outros Crimes Violentos
  ...
  { key: 'roubo a coletivo', category: 'robbery', label: 'Roubo a Coletivo' },
  { key: 'roubo a comercio', category: 'robbery', label: 'Roubo a Estabelecimento Comercial' },
  ...
  { key: 'pirataria fluvial', category: 'robbery', label: 'Pirataria Fluvial / Roubo em Rios' },
  { key: 'roubo de embarcacao', category: 'robbery', label: 'Roubo de Embarcação' },
  { key: 'roubo em embarcacao', category: 'robbery', label: 'Roubo em Embarcação' },
  { key: 'roubo de embarcacoes', category: 'robbery', label: 'Roubo de Embarcações' },
  { key: 'roubo', category: 'robbery', label: 'Roubo' },
  ...
  { key: 'furto de embarcacao', category: 'theft', label: 'Furto de Embarcação' },
  ...
  { key: 'porte de drogas', category: 'drug_related', label: 'Porte / Uso de Drogas' },
  ...
  { key: 'armas apreendidas', category: 'other', label: 'Armas de Fogo Apreendidas', unit: 'armas' },
...
export class SspAmAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas da Segurança Pública do Amazonas (SSP-AM)",
      agency: "SSP-AM",
      frequency: "Mensal",
      coverage: "AM",
      limitations: [
        "Os dados são agregados mensalmente por município ou delegacia pelo Centro Integrado de Análise de Imagens e Estatísticas da SSP-AM.",
        "Dados fluviais (pirataria e roubo de embarcações) são consolidados nas estatísticas de crimes contra o patrimônio nos rios e calhas do Amazonas."
      ]
    };
  }
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SSP-AM",
      dataset: "indicadores_municipais_am",
      url: `https://www.seguranca.am.gov.br/estatisticas/dados-consolidados-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-am-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    // Formato 1: Wide / Matricial Municipal (Colunas de crimes por linha de município)
    const hasMuni = normHeaders.some(h => 
      h.includes('municipio') || h.includes('cidade') || h.includes('cod_ibge') || h.includes('ibge') || h.includes('cisp') || h.includes('dip')
    );
    ...
    const wideCrimeCols = [
      ...
      'pirataria_fluvial', 'roubo_embarcacao'
    ];
    ...
    // Formato 2: Vertical / Registro por Linha
    ...
    // Formato 3: Microdados / Ocorrências Individuais
    ...
    return {
      valid: false,
      error: "Schema incompatível para SSP-AM. Colunas ausentes: cod_ibge / municipio / dip, ano, mes / periodo, colunas de crimes ou natureza/indicador",
      missingColumns: ['municipio / cod_ibge', 'ano / mes', 'natureza / crimes']
    };
  }

  /**
   * Mapeia qualquer natureza criminal ou delito do Amazonas para a taxonomia canônica
   */
  mapCrimeToCanonical(crimeText: string): string {
    ...
    // 1. Verificação exata
    for (const def of AM_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...AM_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
    ...
    if (norm.includes('fluvial') || norm.includes('embarcac') || norm.includes('rio') || norm.includes('pirat')) {
      return 'robbery';
    }
    ...
    if (norm.includes('droga') || norm.includes('entorpecente') || norm.includes('cocaina') || norm.includes('maconha')) {
      return 'drug_related';
    }
  ...
  /**
   * Helper para normalizar o período YYYY-MM
   */
  ...
  /**
   * Helper para extrair código do município ou normalizar nome do AM
   */
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    const rawName = row['municipio'] || row['Municipio'] || row['MUNICIPIO'] || row['cidade'] || row['Cidade'] || row['dip'] || row['DIP'] || '';

    let code = String(rawIbge).trim();
    let name = String(rawName).trim();

    if (code.length === 6 && code.startsWith('13')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const amMunis: Record<string, string> = {
        'manaus': '1302603',
        'parintins': '1303403',
        'itacoatiara': '1301902',
        'manacapuru': '1302504',
        ...
        'maues': '1302900',
        'iranduba': '1302108',
        'sao gabriel da cachoeira': '1303809',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Amazonas' };
  }
  ...
  parseRow(row: any): ParsedRecord[] | ParsedRecord | null {
    ...
    // Identifica se a linha é do formato Vertical (com coluna 'natureza'/'delito')
    ...
    const cvliTotal = this.parseNumeric(normalizedKeys['cvli'] || normalizedKeys['total_cvli'] || normalizedKeys['mortes_violentas']);
    ...
    const rouboTrans = this.parseNumeric(normalizedKeys['roubo_transeunte'] || normalizedKeys['roubo_a_pessoa'] || normalizedKeys['roubo_transeuntes']);
    ...
    const piratariaFluvial = this.parseNumeric(normalizedKeys['pirataria_fluvial'] || normalizedKeys['roubo_embarcacao'] || normalizedKeys['roubo_nos_rios'] || normalizedKeys['roubo_embarcacoes']);
    ...
    const finalRobbery = rouboGen > 0 ? rouboGen : (rouboTrans + rouboCom + rouboRes + rouboCol + piratariaFluvial);
    if (finalRobbery > 0 || (normalizedKeys['roubo_transeunte'] !== undefined) || (normalizedKeys['pirataria_fluvial'] !== undefined)) {
      records.push({
        target: 'indicators',
        data: {
          id: crypto.randomUUID(),
          ...
          municipalityCode: muniCode,
          category: 'robbery',
          sourceCategory: 'Roubos Agregados / CVP / Pirataria Fluvial',
          period,
          value: finalRobbery,
          unit: 'ocorrencias'
        }
