...
export interface SspSeCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const SE_CRIME_DEFINITIONS: SspSeCrimeDefinition[] = [
...
export class SspSeAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas da Secretaria de Estado da Segurança Pública de Sergipe (SSP-SE)",
      agency: "SSP-SE",
      frequency: "Mensal",
      coverage: "SE",
      limitations: [
        "Os dados são compilados pela Coordenadoria de Estatística e Análise Criminal (CEACRIM/SSP-SE).",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SSP-SE",
      dataset: "indicadores_municipais_se",
      url: `https://dados.se.gov.br/dataset/seguranca-publica/indicadores-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-se-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: "Schema incompatível para SSP-SE. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
      missingColumns: ['municipio / cod_ibge', 'ano / mes', 'natureza / crimes']
    };
  }

  /**
   * Mapeia qualquer natureza criminal de Sergipe para a taxonomia canônica
   */
  mapCrimeToCanonical(crimeText: string): string {
    ...
    // 1. Verificação exata
    for (const def of SE_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...SE_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    return { code: code || 'UNKNOWN', name: name || 'Sergipe' };
  }
