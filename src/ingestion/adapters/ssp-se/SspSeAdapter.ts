...
export class SspSeAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas Criminais SSP-SE",
      agency: "Secretaria de Segurança Pública (SE)",
      frequency: "Mensal",
      coverage: "SE",
      limitations: []
    };
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SSP-SE",
      dataset: "Indicadores_Criminais_SE",
      url: `https://www.seguranca.se.gov.br/dados/${currentYear}.csv`,
  ...
  parseRow(row: any): ParsedRecord[] | ParsedRecord | null {
    ...
    records.push({
      target: 'indicators',
      data: { id: crypto.randomUUID(), sourceId: 'SSP-SE', stateCode: 'SE', municipalityName: municipio, category: cat, sourceCategory: natureza, period: `${ano}-${mes}`, value: total, unit: 'ocorrencias' }
    });
    return records;
  }
}
