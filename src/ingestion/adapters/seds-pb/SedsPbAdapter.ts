...
export interface SedsPbCrimeDefinition {
  key: string;
  category: string;
  label: string;
  unit?: string;
...
export const PB_CRIME_DEFINITIONS: SedsPbCrimeDefinition[] = [
  ...
  // Crimes Violentos Patrimoniais (CVP) / Roubos
...
export class SedsPbAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Estatísticas da Secretaria de Estado da Segurança e da Defesa Social da Paraíba (SEDS-PB)",
      agency: "SEDS-PB",
      frequency: "Mensal",
      coverage: "PB",
      limitations: [
        "Os dados são consolidados mensalmente por município ou AISP pela Gerência Executiva de Estatística e Análise Criminal (GEAC/SEDS-PB).",
        "Indicadores de CVLI integram Homicídio Doloso, Feminicídio, Latrocínio e Lesão Corporal Seguida de Morte (Lei 13.964/2019)."
      ]
    };
  }
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    return {
      source: "SEDS-PB",
      dataset: "indicadores_municipais_pb",
      url: `https://www.seguranca.pb.gov.br/estatisticas/dados-consolidados-${currentYear}.csv`,
      version,
      checksum: crypto.createHash('sha256').update(`seds-pb-${version}`).digest('hex')
    };
  }
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    return {
      valid: false,
      error: "Schema incompatível para SEDS-PB. Colunas ausentes: cod_ibge / municipio, ano, mes / periodo, colunas de crimes ou natureza/indicador",
      missingColumns: ['municipio / cod_ibge', 'ano / mes', 'natureza / crimes']
    };
  }

  /**
   * Mapeia qualquer natureza criminal ou delito da Paraíba para a taxonomia canônica
   */
  mapCrimeToCanonical(crimeText: string): string {
    ...
    // 1. Verificação exata
    for (const def of PB_CRIME_DEFINITIONS) {
      if (norm === def.key) {
        return def.category;
      }
    }
    ...
    const sortedDefs = [...PB_CRIME_DEFINITIONS].sort((a, b) => b.key.length - a.key.length);
  ...
  /**
   * Helper para extrair código do município ou normalizar nome da PB
   */
  private extractMunicipality(row: any): { code: string; name: string } {
    ...
    const rawName = row['municipio'] || row['Municipio'] || row['MUNICIPIO'] || row['cidade'] || row['Cidade'] || row['aisp'] || '';

    let code = String(rawIbge).trim();
    let name = String(rawName).trim();

    if (code.length === 6 && code.startsWith('25')) {
      code = code + '0';
    }

    if (!code || code === '0') {
      const canon = GeoNormalizationService.canonicalizeMunicipalityName(name);
      const pbMunis: Record<string, string> = {
        'joao pessoa': '2507507',
    ...
    return { code: code || 'UNKNOWN', name: name || 'Paraíba' };
  }
