...
const BA_CRIME_DEFINITIONS: BaCrimeMapping[] = [
  ...
  { key: 'roubo comercial', category: 'robbery', subcategory: 'roubo_comercio', unit: 'ocorrencias' },
...
const BA_MONTH_NAMES_MAP: Record<string, number> = {
...
/**
 * Adapter oficial para SSP-BA (Secretaria da Segurança Pública do Estado da Bahia).
 * Suporta os relatórios estatísticos municipais e de CVLI consolidados pela SIAP/CEVAL.
 */
export class SspBaAdapter extends BaseAdapter {
  metadata(): AdapterMetadata {
    return {
      name: "Secretaria da Segurança Pública da Bahia - SSP-BA",
      agency: "SSP-BA",
      frequency: "Mensal",
      coverage: "BA",
      limitations: [
        "Estatísticas e dados de CVLI consolidados pela Coordenação de Estatística e Avaliação Operacional (CEVAL / SIAP)",
        "Cobertura total de 100% dos 417 municípios do Estado da Bahia",
  ...
  async discover(): Promise<DiscoveryResult> {
    ...
    const url = `https://www.ssp.ba.gov.br/estatisticas`;

    return {
      source: "SSP-BA",
      dataset: "indicadores_municipais_ba",
      url,
      version,
      checksum: crypto.createHash('sha256').update(`ssp-ba-${version}`).digest('hex')
    };
  }

  async download(destinationPath: string): Promise<string> {
    console.log(`[SSP-BA Adapter] Ingestão pronta para ${destinationPath}`);
  ...
  validateSchema(headers: string[]): SchemaValidationResult {
    ...
    // Formato 1: Wide (colunas de crimes e CVLI)
    const hasWideCols = cleanHeaders.some(h => 
      ['cvli', 'homicidio_doloso', 'homicidio doloso', 'latrocinio', 'feminicidio', 'roubo_veiculo', 'roubo de veiculo', 'furto_veiculo', 'furto de veiculo', 'roubo', 'furto', 'estupro', 'trafico_drogas'].includes(h)
    );
    ...
    return {
      valid: false,
      error: `Schema incompatível para SSP-BA. Colunas ausentes: ${missing.join(', ')}`,
  ...
  mapCrimeToCanonical(naturezaStr: string): CanonicalCategory {
    ...
    const found = BA_CRIME_DEFINITIONS.find(m => m.key === clean || clean.includes(m.key));
    ...
    if (clean.includes('roubo') || clean.includes('transeunte') || clean.includes('assalto') || clean.includes('onibus') || clean.includes('coletivo')) return 'robbery';
  ...
  parseRow(row: any): ParsedRecord[] | null {
    ...
    if (BA_MONTH_NAMES_MAP[cleanMesStr]) {
      mesNum = BA_MONTH_NAMES_MAP[cleanMesStr];
    } else {
    ...
    if (rawNatureza) {
      ...
      const def = BA_CRIME_DEFINITIONS.find(d => d.key === cleanNaturezaKey || cleanNaturezaKey.includes(d.key));
    ...
    // 1. Homicide (CVLI ou soma consolidada de crimes violentos letais)
    ...
    const latroVal = this.extractNumericValue(normalizedRow, 'latrocinio');
    ...
    // 2. Vehicle Robbery
    const vehRob = this.extractNumericValue(normalizedRow, 'roubo_veiculo') || this.extractNumericValue(normalizedRow, 'roubo_de_veiculo') || this.extractNumericValue(normalizedRow, 'roubo_auto');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
    ...
    // 3. Vehicle Theft
    const vehTheft = this.extractNumericValue(normalizedRow, 'furto_veiculo') || this.extractNumericValue(normalizedRow, 'furto_de_veiculo') || this.extractNumericValue(normalizedRow, 'furto_auto');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
    ...
    // 4. Robbery (Transeunte / Ônibus / Comércio / Residência / Geral)
    const robBus = this.extractNumericValue(normalizedRow, 'roubo_onibus') || this.extractNumericValue(normalizedRow, 'roubo_coletivo');
    const robCom = this.extractNumericValue(normalizedRow, 'roubo_comercio') || this.extractNumericValue(normalizedRow, 'roubo_comercial');
    const robRes = this.extractNumericValue(normalizedRow, 'roubo_residencia');
    const robTran = this.extractNumericValue(normalizedRow, 'roubo_transeunte');
    const robGer = this.extractNumericValue(normalizedRow, 'roubo') || this.extractNumericValue(normalizedRow, 'roubo_geral');
    const totalRobbery = (robBus + robCom + robRes + robTran) > 0 ? (robBus + robCom + robRes + robTran) : robGer;
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
    ...
    // 5. Theft (Furto Geral)
    const theftVal = this.extractNumericValue(normalizedRow, 'furto') || this.extractNumericValue(normalizedRow, 'furto_geral') || this.extractNumericValue(normalizedRow, 'furto_qualificado');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
    ...
    const drugVal = this.extractNumericValue(normalizedRow, 'trafico_drogas') || this.extractNumericValue(normalizedRow, 'trafico_de_drogas') || this.extractNumericValue(normalizedRow, 'entorpecentes');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
    ...
    const armsVal = this.extractNumericValue(normalizedRow, 'apreensao_armas') || this.extractNumericValue(normalizedRow, 'apreensao_de_armas') || this.extractNumericValue(normalizedRow, 'armas_fogo');
    records.push({
      target: 'indicators',
      data: {
        id: crypto.randomUUID(),
