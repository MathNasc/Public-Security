...
if (!code.includes('import { X,')) {
  code = code.replace(/import \{ CheckCircle2/, 'import { X, CheckCircle2');
}

// 2. We need to create a Modal for the password
...
if (!code.includes('const showToast =')) {
  code = code.replace(
...
// 3. Update the triggerCrawler to open the modal instead of using prompt
code = code.replace(
  /const triggerCrawler = async \(\) => \{[\s\S]*?try \{[\s\S]*?const token = prompt\("Insira a senha de Admin \(ADMIN_SECRET\):"\);[\s\S]*?if \(\!token\) \{[\s\S]*?setIsUploading\(false\);[\s\S]*?return;[\s\S]*?\}[\s\S]*?const response = await fetch\("\/api\/admin\/automation\/trigger-all", \{[\s\S]*?method: "POST",[\s\S]*?headers: \{ 'Authorization': \`Bearer \$\{token\}\` \} \n\s+\}\);/,
...
// Fix await fetchSources() bug if present
code = code.replace(/await fetchSources\(\);/g, 'fetchSources();');

// 4. Inject the Toast and Password Modal UI right after the main container div
...
if (!code.includes('isPasswordModalOpen')) {
  code = code.replace(
